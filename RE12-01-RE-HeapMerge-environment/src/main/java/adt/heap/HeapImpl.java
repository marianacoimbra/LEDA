...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		...
		if (left <= this.index && comparator.compare(this.heap[position], this.heap[left]) > 0) {
			maior = position;
		} else {
			maior = left;
		}
		if (right <= this.index && comparator.compare(heap[right], heap[maior]) > 0) {
			maior = right;
		}
	...
	public T[] heapsort(T[] array) {
		if (array != null) {
			HeapImpl<T> newHeap = new HeapImpl<T>((o1, o2) -> o1.compareTo(o2));

			newHeap.buildHeap(array);

			T[] aux = (T[]) new Comparable[array.length];
			for(int i = array.length - 1; i >= 0; i --) {
				aux[i] = newHeap.extractRootElement();
			}
			return aux;
		}
		return null;
